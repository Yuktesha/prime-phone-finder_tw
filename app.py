...
if os.path.exists(frontend_path):
    logger.info(f"Frontend directory exists: {frontend_path}")
    logger.info(f"Frontend directory contents: {os.listdir(frontend_path)}")
else:
    logger.error(f"Frontend directory does not exist: {frontend_path}")
    # 嘗試在當前目錄中查找 index.html
    if os.path.exists(os.path.join(app_path, 'index.html')):
        logger.info("Found index.html in the current directory")
        frontend_path = app_path
    else:
        logger.error("Could not find frontend files")
...
def serve_static(path):
    ...
    # 嘗試從前端目錄提供文件
    try:
        logger.info(f"Trying to serve: {path} from {frontend_path}")
